    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                ...
                assert place.ant is None, f"Two ants in {place}"
            # END Problem 8b
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not curr_place.is_hive:
            if (
                random_bee(curr_place.bees) != None
                and self.lower_bound <= count <= self.upper_bound
            ):
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        elif target_bee != None:
            Bee.reduce_health(target_bee, target_bee.health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        if other.is_container or self.ant_contained != None:
            return False
        else:
            return True
        # END Problem 8a
    ...
    def remove_ant(self, ant):
        if self.ant_contained is not ant:
            assert False, f"{self} does not contain {ant}"
        self.ant_contained = None
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        new_place = self.place.exit
        while new_place != None:
            if new_place.ant != None:
                if (
                    new_place.ant.is_container == True
                    and new_place.ant.ant_contained != None
                ):
                    Ant.double(new_place.ant.ant_contained)
                Ant.double(new_place.ant)
...
class GameState:
    ...
    def __str__(self):
        status = f" (Food: {self.food}, Time: {self.time})"
    ...
    for tunnel in range(tunnels):
        ...
        for step in range(length):
            if moat_frequency != 0 and (step + 1) % moat_frequency == 0:
                exit = Water(f"water_{tunnel}_{step}", exit)
            else:
                exit = Place(f"tunnel_{tunnel}_{step}", exit)
