...
def about(subject):
    ...
    def contain(paragraphs):
        word = split(lower(remove_punctuation(paragraphs)))
            ...
            if i < len(subject):
                if j < len(word):
                    if subject[i] == word[j]:
                        ...
                        return tracker(i, j + 1)
                else:
                    return tracker(i + 1, 0)
            else:
                return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    def accuracy_helper(numerator, denominator):
        if len(typed_words) == 0:
            ...
            return 0.0
        elif denominator < len(typed_words):
            if denominator < len(source_words):
                if typed_words[denominator] == source_words[denominator]:
                    numerator += 1
            denominator += 1
            return accuracy_helper(numerator, denominator)
        return (numerator / denominator) * 100

    return accuracy_helper(0, 0)
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    i, lim, flag = 0, limit, 1
    while i < len(word_list):
        if word_list[i] == typed_word:
            return word_list[i]
        else:
            difference = diff_function(typed_word, word_list[i], lim)
            if difference <= lim:
                if difference != lim:
                    word = word_list[i]
                    lim = difference
                if difference == lim and flag and lim == limit:
                    flag = 0
                    word = word_list[i]
                    lim = difference
        i += 1
    if lim == limit and flag == 1:
        return typed_word
    else:
        return word
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    # END
        ...
        # BEGIN
        count = min(add, remove, substitute)
    return count
    # END
...
def report_progress(typed, source, user_id, upload):
    ...
    """
    # BEGIN PROBLEM 8
    i, num, den = 0, 0, len(source)
    while i < min(len(source), len(typed)):
        if typed[i] == source[i]:
            num += 1
            i += 1
        else:
            i = min(len(source), len(typed))
    upload({"id": user_id, "progress": num / den})
    return num / den
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in timestamps_per_player:
        words_time.append([i[j] - i[j - 1] for j in range(1, len(i))])
    return match(words, words_time)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, player_indices[j], i) < fast:
                fast = time(match, player_indices[j], i)
                index = j
        fastest[index].append(get_word(match, i))
    return fastest
    # END PROBLEM 10
